 ...
 * NativeWind resolves conflicting utilities by source order, so anything you
 * pass wins over the role default.
 */
...
export function BodyText({ size = 'base', className = '', ...rest }: BodyTextProps) {
  return <Text className={`${bodyClasses[size]} ${className}`} {...rest} />;
}
...
export function Eyebrow({ className = '', ...rest }: EyebrowProps) {
  return (
    <Text
      className={`font-condensed text-xs uppercase leading-none tracking-eyebrow text-text-muted ${className}`}
      {...rest}
    />
  );
}
...
export function Stat({ className = '', ...rest }: StatProps) {
  return (
    <Text
      className={`font-condensed-bold text-4xl leading-none text-text-heading ${className}`}
      {...rest}
    />
  );
}
